...
def _orchestrator_contract(config, *, preset: str | None = None) -> str:
    ...
    lines = [
        ...
        "For parallel or staged work, start independent workers concurrently and continue useful non-overlapping host work while they run.",
        "Before final synthesis, ensure every worker result required by the answer has reached a terminal or idle state. Never conclude merely because required work is still pending.",
        "Consume agent activity incrementally with sequence cursors. Preserve each worker's complete final result; summaries and progress events are supplementary and must not replace it.",
        "Provider-native subagents are allowed. Every discovered child agent must be mirrored into the CortexRelay agent tree with its provider session ID, parent, state, events, messages, and available workspace or transcript references.",
        "Native children remain subject to CortexRelay depth limits, budgets, access policy, scheduler controls, workspace boundaries, and quality gates.",
        "Do not create invisible provider-side work when an observable CortexRelay-managed alternative exists.",
        "For staged write workflows, preserve exact dependency and workspace lineage so downstream workers consume the immutable artifact produced upstream.",
        "Inspect a worker diff before applying it. Never apply or discard a worker worktree without explicit user intent.",
        "Respect configured budgets, quality gates, access policies, recursion limits, scheduler queues, and workspace boundaries.",
        "",
        "Configured roles:",
    ]
