...
State is external to the Git checkout and keyed by the resolved workspace path. Workflow-task results remain persisted through RunStore, including dedicated UTF-8 final-output files for lossless paging. Persistent agent control state is stored separately in one canonical transactional SQLite/WAL database: sessions, indexed provider handles, child topology, atomic event/message cursors, completed-turn results, and renewable ownership leases. Provider hosts may isolate their own runtime state, but embedded CortexRelay transports receive the canonical `CORTEX_RELAY_STATE_DIR` explicitly so host-local temporary directories cannot create a split-brain control plane. A new MCP process can inspect durable sessions without process-local futures and can reconcile expired active leases to an interrupted state. The CLI status snapshot joins both stores by resolved workspace, so `status --watch` is a unified view over task lifecycle and agent-session lifecycle, including mirrored native children and recent semantic activity.
...
The local SQLite AgentStore is a single-host production backend, not a distributed coordination database. The `AgentStoreBackend` protocol defines the provider-neutral persistence contract and `ProviderRegistry` accepts any structurally compatible backend, so the control plane is not hard-wired to SQLite. A future PostgreSQL or managed-database implementation can provide the same session identity, cursor, topology, result, lease, health, and retention semantics for multiple CortexRelay replicas without subclassing the SQLite implementation.
