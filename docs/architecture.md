...
State is external to the Git checkout and keyed by the resolved workspace path. Workflow-task results remain persisted through RunStore, including dedicated UTF-8 final-output files for lossless paging. Persistent agent control state is stored separately in transactional SQLite/WAL tables: sessions, indexed provider handles, child topology, atomic event/message cursors, completed-turn results, and renewable ownership leases. A new MCP process can inspect durable sessions without process-local futures and can reconcile expired active leases to an interrupted state. The CLI status snapshot joins both stores by resolved workspace, so `status --watch` is a unified view over task lifecycle and agent-session lifecycle, including mirrored native children and recent semantic activity.
...
The local SQLite AgentStore is a single-host production backend, not a distributed coordination database. The storage boundary is deliberately kept behind AgentStore so a future PostgreSQL or managed-database implementation can provide the same session/event/message/lease semantics for multiple CortexRelay replicas.
