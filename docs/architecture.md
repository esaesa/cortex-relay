...
The local SQLite AgentStore is a single-host production backend, not a distributed coordination database. ProviderRegistry accepts an injected AgentStore backend so the storage boundary is not hard-wired to SQLite; a future PostgreSQL or managed-database implementation can provide the same session/event/message/lease semantics for multiple CortexRelay replicas.
